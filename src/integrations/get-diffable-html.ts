...
function getDiffableHTML(container: Node, options?: DiffOptions): string {
    ...
    const ignoreAttributesForTags = options?.ignoreAttributes
        ? (options.ignoreAttributes.filter(
              (e) => typeof e !== 'string'
          ) as IgnoreAttributesForTags[])
        : []
    const ignoreTags = [...(options?.ignoreTags || []), ...DEFAULT_IGNORE_TAGS]
    const ignoreChildren = options?.ignoreChildren || []
    const ignoreShadowChildren = [
        ...(options?.ignoreShadowChildren || []),
        ...DEFAULT_IGNORE_SHADOW_CHILDREN,
    ]
    ...
    function walk(current: Node, depth: number) {
        ...
        function getAttributes(el: any): Attr[] {
            ...
            if (el.properties) {
                ...
                propertyNames = [...props.keys()].filter(
                    (p: string) => props.get(p).attribute !== false
                )
                ignoreAttributeNames = [...props.entries()].map(([name, p]) =>
                    typeof p.attribute === 'string' ? p.attribute : name
                )
            } else {
                return attributes
            }

            if (stripUndefinedAttributes)
